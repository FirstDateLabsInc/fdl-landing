// Force dynamic rendering - never cache shared results (security)
...
import { getLockedContentById } from "@/lib/quiz/data/archetypes/selectors.server";
...
export default async function PublicSharePage({ params }: Props) {
  ...
  // Validate slug format (nanoid 21 chars, URL-safe alphabet: A-Za-z0-9_-)
  const slugRegex = /^[A-Za-z0-9_-]{21}$/;
  if (!slugRegex.test(publicSlug)) {
    notFound();
  }

  // Fetch by public_slug (bearer token authorization - existence = permission)
  const supabase = getSupabaseServer();
  const { data, error } = await supabase
    .from("quiz_results")
    .select("id, archetype_slug, scores, created_at, public_slug")
    .eq("public_slug", publicSlug)
    .single();

  // No result or no public_slug = not found
  if (error || !data || !data.public_slug) {
    notFound();
  }
  ...
  // Get PUBLIC archetype data (always needed)
  const archetype = getPublicArchetypeById(data.archetype_slug);
  if (!archetype) {
    notFound();
  }

  // Get LOCKED content (server-only, never in client bundle)
  const lockedContent = getLockedContentById(data.archetype_slug);
  if (!lockedContent) {
    notFound();
  }
